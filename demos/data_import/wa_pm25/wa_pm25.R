...
df.pm25 <- bind_rows(lapply(c("81", "109", "110"), function(x) {
  url <-
    paste0("https://fortress.wa.gov/ecy/enviwa/DynamicTable.aspx?G_ID=",
           x)
  df <- read_html(url) %>%
    html_node("table#C1WebGrid1") %>%
    html_table() %>%
    .[- (1:2), ] %>%
    mutate_at(- (1:2), as.numeric)
  df %>%
    mutate(pm25 = rowSums(df[, 3:ncol(df)], na.rm = TRUE)) %>%
    select(X1, X2, pm25) %>%
    set_names(c("site", "timestamp", "pm25")) %>%
...
df.o3 <-
  ...
  mutate_at(- (1:2), as.numeric) %>%
  select(X1, X2, X3) %>%
  set_names(c("site", "timestamp", "o3")) %>%
  mutate(timestamp = as.POSIXct(timestamp, format = time_format, tz = tz)) %>%
  tidyr::drop_na() %>%
  mutate(aqi_o3 = con2aqi("o3", o3, "1h"))

...
links <- read_html(url) %>%
  html_nodes(xpath = "//div[@class='syndicate']/ul/li/a") %>%
  html_attr("href")
...
df <- df %>%
  mutate(values = gsub("Case Count", "Reported Cases", values)) %>%
  separate(values, c("key", "value"), sep = ": ") %>%
  spread(key, value) %>%
  mutate_at(vars(-ind), as.integer) %>%
...
df_summ <- df %>%
  ...
  filter(States %in% df_states_summ$States) %>%
  as_tibble()
...
identical(df_summ, df_states_summ)

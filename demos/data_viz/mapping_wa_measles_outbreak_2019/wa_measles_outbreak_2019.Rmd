...
locations$popup <- paste(
  '<dl><dt>2019 Measles Exposure Site</dt><dd>', locations$site, '</dd></dl>')

icon.fa <- makeAwesomeIcon(icon = "fa-exclamation-triangle ", 
...
g2 <- g %>% addAwesomeMarkers(data = locations, ~lon, ~lat, 
                    popup = ~popup, icon = icon.fa) %>% 
  setView(-122.5487, 45.65366, zoom = 11) %>% 
  addControl(title, position = "topleft", className="map-title")
```

## Clark County Measles Outbreak and Immunization Coverage
...
You can download a [PDF](https://www.oregon.gov/oha/PH/PREVENTIONWELLNESS/VACCINESIMMUNIZATION/Documents/RatesK12ImmandExmptbyCounty.pdf) containing the necessary data.

This PDF is linked from the [Oregon School Immunization Data](https://www.oregon.gov/oha/PH/PREVENTIONWELLNESS/VACCINESIMMUNIZATION/Pages/researchschool.aspx) page.

Use the "Complete for All Required Immunizations (%)" values for plotting, or 
perhaps the "1 Dose Measles, Mumps and Rubella" and "2 Doses Measles Vaccine" 
values, whichever you prefer.

To extract the data from the tables in the PDF, you may wish to use the 
`tabulizer` package.

An example of using this package can be found in this script:

* [WA Counties HepC Choropleth](../mapping_with_ggplot2/wa_counties_hepc_choropleth.R)

Alternatively, Adobe Acrobat Pro can export to Excel. Or you can copy/paste from 
Adobe Acrobat Reader DC to MS Excel, a text editor, or any other application 
that works for you. Using `tablulizer` (or equivalent) is preferred as it is a 
more automated and reproducible approach.

## Exercise 2: `ggmap`

Create a map with `ggmap` which plots the points of either of these two sets 
of exposure locations. (Choose to do either A. or B. or both.)
...
The points of the exposure locations are provided at this link: [Pierce County Measles Investigation](https://www.tpchd.org/healthy-people/diseases/measles/pierce-county-measles-investigation)

B. Oregon

The Multomah Health Officer published a list of [exposure locations](https://multco.us/health-officer/measles-outbreak-winter-2019-oregon-exposures) for the Winter 2019 measles outbreak.
...
to. Since there are only a few of them, this can be done manually without too 
...
* [King County Measles Exposure Sites](../mapping_wth_ggmap/king_co_measles_exposure_sites_map.R)
...
was used for Washington the example earlier in this presentation. That is,
...
provided for you: [or_k12_immunization_coverage.csv](../mapping_with_leaflet/or_k12_immunization_coverage.csv).

The code which generated this file is in the next section of this document.

Bonus points if you add markers for the exposure sites from Exercise 2-B.

## Exercise 3: `leaflet`, cont.

```{r, eval=FALSE}
# Geocode the Oregon K-12 school locations
pacman::p_load(readxl, ggmap)

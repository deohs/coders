...
wa_coverage <- wa_coverage %>% 
  mutate(Popup = paste0('<dl>', 
    ...
    '<dt>', 'Percent Exempt', '</dt><dd>', htmlEscape(`Exempt`), '</dd>', 
    '</dl>'))
```
...
locations$popup <- paste(
  '<dl><dt>2019 Measles Exposure Site</dt><dd>', 
  htmlEscape(locations$site), '</dd></dl>')
...
```{r, echo=FALSE}
knitr::kable(wa_coverage_by_dist %>% 
               filter(County == 'Clark') %>% 
               arrange(Percent))
```
...
g3 <- leaflet(data = wa_coverage) %>% 
  ...
  addCircleMarkers(group = "schools", lng = ~lon, lat = ~lat, 
    color = ~pal(Pct), popup = ~Popup, label = ~htmlEscape(`School Name`),
    radius = ~sqrt(Enrollment) * .3) %>% 

...
We view the results on the next slide.

## Coverage by school district

```{r}
knitr::kable(wa_coverage_by_dist %>% filter(County == 'Clark') %>% arrange(Percent))
```

## Get school district polygons

We can import a shapefile from [data.gov](https://catalog.data.gov/dataset/tiger-line-shapefile-2013-state-washington-current-unified-school-districts-) to see which schools are in what district.

```{r}
# Load packages.
pacman::p_load(rgdal, rgeos)

# Get and import shapefile.
shp_fn <- 'tl_2013_53_unsd.zip'
if (!file.exists(shp_fn)) {
  url <- 'http://www2.census.gov/geo/tiger/TIGER2013/UNSD/tl_2013_53_unsd.zip'
  download.file(url, destfile = 'tl_2013_53_unsd.zip')
}
unzip(shp_fn, exdir = "gis")
shapeData <- readOGR("gis")
...
# Filter to include only Clarke County school districts.
...
# Create district center labels and popup
centers <- data.frame(gCentroid(shapeData_clark, byid = TRUE))
centers$NAME <- shapeData_clark$NAME
shapeData_clark$popup <- paste(
  "<dl><dt>District:</dt?<dd>", shapeData_clark$NAME, "</dd>",
  "<dt>Percent Complete:</dt><dd>", shapeData_clark$Percent, "</dd></dl>")
```
...
g3 <- leaflet(data = wa_coverage) %>% setView(-122.5487, 45.65366, zoom = 11) %>% 
  addControl(title, position = "topleft", className ="map-title") %>% 
  ...
  addPolygons(group = "districts", data = shapeData_clark, weight = 4,
    ...
    popup = ~popup, fillOpacity = 0.3, fillColor = pal(shapeData_clark$Pct)) %>% 
  addLabelOnlyMarkers(data = centers, lng = ~x, lat = ~y, label = ~NAME, 
    labelOptions = labelOptions(textsize='15px', opacity = 0.6,
      noHide = TRUE, direction = 'top', textOnly = FALSE)) %>% 
  addCircleMarkers(lng = ~lon, lat = ~lat, 
    color = ~pal(Pct), popup = ~Popup, 
    radius = ~sqrt(Enrollment) * .3, group = "schools") %>% 
  addAwesomeMarkers(data = locations, ~lon, ~lat, 
    popup = ~popup, icon = icon.fa, group = "exposure") %>% 
  addLayersControl(baseGroups = c("tiles"), 
    overlayGroups = c("legend", "districts", "schools", "exposure"),

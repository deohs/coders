...
We can fix them by geocoding with the `geocode()` function. This code has been 
structured so that if you don't have your key stored in the file 
`~/google_api.key`, then the geocoding step will just be skipped.
...
if (file.exists(key_fn)) {
  register_google(key = readLines(key_fn))
  if (has_google_key()) {
    new_locations <- wa_coverage %>% 
      filter(grepl(schools_regex, `School Name`)) %>% pull(`School Name`) %>% 
      geocode()
    wa_coverage[grepl(schools_regex, 
      wa_coverage$`School Name`), c('lon', 'lat')] <- new_locations
  }
}
```

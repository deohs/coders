...
# Create district center labels and popups.
...
g3 <- leaflet(data = wa_coverage) %>% 
  ...
  addLabelOnlyMarkers(data = centers, lng = ~x, lat = ~y, label = ~NAME, 
    labelOptions = labelOptions(textsize='15px', opacity = 0.6, noHide = TRUE, 
      direction = 'top', textOnly = FALSE), group = "districts") %>% 
  addCircleMarkers(group = "schools", lng = ~lon, lat = ~lat, 
...
add polygons for the Oregon school [districts](https://spatialdata.oregonexplorer.info/geoportal/details;id=1270fe6e833f4d0eabacc71300069738).
